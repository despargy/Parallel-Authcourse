...
#define COMPARE_MIN (1 << 12) - 1
#define CALL_MIN (1 << 8) - 1
...
init();
  ...
  printf("Parallel Imperative wall clock time using CilkPlus = %f\n", seq_time);
  ...
  printf("Parallel Recursive wall clock time using CilkPlus = %f\n", seq_time);
...
void PbitonicMerge(int lo, int cnt, int dir) {
	if (cnt>1) {
		...
		if (cnt >CALL_MIN){
			cilk_spawn PbitonicMerge(lo, k, dir);
			cilk_spawn PbitonicMerge(lo+k, k, dir);
			cilk_sync;
		} else{
...
void PrecBitonicSort(int lo, int cnt, int dir) {
  if (cnt>=BASE) {
    int k=cnt/2;
    cilk_spawn PrecBitonicSort(lo, k, ASCENDING);
    cilk_spawn PrecBitonicSort(lo+k, k, DESCENDING);
    cilk_sync;
